...
function parseNumber(number, dictionary) {
  if (number.includes(dictionary.decimalSeparator)) {
    return (
      getText(number.split(dictionary.decimalSeparator)[0], dictionary) +
      dictionary.decimalConnector +
      getText(number.split(dictionary.decimalSeparator)[1], dictionary)
    );
  }
  return getText(number, dictionary);
}
